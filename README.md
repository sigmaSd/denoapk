...
your web code", so denapk does the same thing with an Android WebView.

```
denapk build ~/my-app        # -> dist/my-app.apk
...
`web/` directory first; denapk will tell you if it can't find one.
...
cross-origin requests are rewritten to `/__denapk/proxy/<encoded-url>` with
their headers moved under an `x-denapk-h-` prefix, which the browser sends
...
`javac` + `d8` build the shell's `classes.dex`, cached under `~/.cache/denapk`
...
**fully qualified** (`dev.denapk.shell.MainActivity`). A leading-dot name
resolves against the manifest `package`, which changes per app — Android would
then look for a class the dex doesn't contain.

## Toolchain

`denapk doctor` reports status. On first run it downloads the Android
command-line tools into `~/.cache/denapk/android-sdk` and installs
...
`~/.cache/denapk/debug.keystore` — fine for sideloading and `adb install`, not
