...
A `deno desktop` app is a `deno.json` with a `desktop.app` block plus a `web/`
directory. That's also all denoapk needs — nothing extra to add.
...
Sometimes an app needs something a browser genuinely cannot do at all — not a
header/CORS problem `fetch` can be shimmed around, but running an actual system
...
runs whatever binary and args it's given, with **no allowlist**: any command the
app's own JS can name, it can run. That's a deliberately wider trust boundary
than the fetch proxy (which is at least restricted to HTTP/HTTPS), chosen so any
app can reuse this for whatever native primitive it needs without denoapk
maintaining a list of "known-safe" commands.

`exec` and `execStream` reach the host through genuinely different mechanisms,
and it matters which one an app needs:

- `exec` runs to completion and returns one result — a normal `fetch()` call
  under the hood on both platforms.
- `execStream` is for a long-running or unbounded command (continuous `ping`,
  not `ping -c 1`) and needs to actually stream. On a Deno host that's still
  plain `fetch()` — a chunked response body streams there without issue. On
  Android it can't be: `shouldInterceptRequest`'s `WebResourceResponse` reads
  its `InputStream` to EOF internally before the page's `fetch()` sees anything,
  so an unbounded process would never deliver a byte (confirmed on-device, not
  assumed). Android instead uses an `addJavascriptInterface` bridge that pushes
  each chunk to the page via `evaluateJavascript()` as it arrives — real
...
binary-patching the AXML string pool in a prebuilt template APK. `aapt2
link` on
